...
namespace GT {
    ...
    protected:
        ...
        void search_subtree(tree_node_t *data_node, double dot_product);
        void search_single_data_point(tree_node_t *data_node, tree_node_t *query_node, double dot_product);
        void search_single_query_point(tree_node_t *data_node, tree_node_t *query_node, double dot_product);
        void search_double_subtree(tree_node_t *data_node, tree_node_t *query_node, double dot_product);
    };
    ...
    tree_node_t build_index_tree(sparse_vec_t *matrix, size_t __size, size_t __start_index);
...
GT::GroupTestingNN::GroupTestingNN(string &__file_directory, double __threshold) {
    ...
    this->root = make_unique<GT::tree_node_t>(GT::build_index_tree(this->data_set.data(), this->data_set.size(), 0));
...
template <size_t N = 0> pair<double, size_t> GT::GroupTestingNN::search(void) {
    ...
    for (this->query_index = 0; this->query_index < this->query_set.size(); this->query_index++) {
        ...
        this->num_dot_products = (1 << N);
        auto start = high_resolution_clock::now();
        for (size_t i = 0; i < (1 << N); i++) {
            double dot_product = nodes[i]->sum_vec * this->query_set[this->query_index];
            if (dot_product >= this->threshold) {
                this->search_subtree(nodes[i], dot_product);
            }  
        }
...
template <size_t N = 0, size_t Q = 0> pair<double, size_t> GT::GroupTestingNN::double_search(void) {
    ...
    unique_ptr<GT::tree_node_t> query_root = make_unique<GT::tree_node_t>(
            GT::build_index_tree(this->query_set.data(), this->query_set.size(), 0));
    array<GT::tree_node_t*, 1 << Q> query_nodes = GT::get_nodes_at_level<Q>(query_root.get());
    for (size_t i = 0; i < (1 << N); i++) {
        for (size_t j = 0; j < (1 << Q); j++) {
            double dot_product = nodes[i]->sum_vec * query_nodes[j]->sum_vec;
            if (dot_product >= this->threshold) {
                this->search_double_subtree(nodes[i], query_nodes[j], dot_product);
            }
        }
    }
    auto stop = high_resolution_clock::now();
...
pair<double, double> GT::GroupTestingNN::get_precision_and_recall(void) {
    assert(this->search_res.size() == this->naive_res.size());
    assert(this->search_res.size() == this->query_set.size());
...
void GT::GroupTestingNN::search_subtree(GT::tree_node_t *data_node, double dot_product) {
    ...
    double left_dot_product = data_node->left->sum_vec * this->query_set[this->query_index];
    dot_product -= left_dot_product;
    this->num_dot_products++;
    if (left_dot_product >= threshold) {
        this->search_subtree(data_node->left.get(), left_dot_product);
    }
    if (dot_product >= threshold) {
        this->search_subtree(data_node->right.get(), dot_product);
    }
}

void GT::GroupTestingNN::search_single_data_point(GT::tree_node_t *data_node, 
        GT::tree_node_t *query_node, double dot_product) {
    ...
    double left_dot_product = data_node->sum_vec * query_node->left->sum_vec;
    dot_product -= left_dot_product;
    this->num_dot_products++;
    if (left_dot_product >= threshold) {
        this->search_single_data_point(data_node, query_node->left.get(), left_dot_product);
    }
    if (dot_product >= threshold) {
        this->search_single_data_point(data_node, query_node->right.get(), dot_product);
    }
}

void GT::GroupTestingNN::search_single_query_point(GT::tree_node_t *data_node, 
        GT::tree_node_t *query_node, double dot_product) {
    if (data_node->left == nullptr) {
        if (data_node->sum_vec * query_node->sum_vec >= threshold) {
            this->double_search_res[query_node->index].push_back(data_node->index);
    ...
    double left_dot_product = data_node->left->sum_vec * query_node->sum_vec;
    dot_product -= left_dot_product;
    this->num_dot_products++;
    if (left_dot_product >= threshold) {
        this->search_single_query_point(data_node->left.get(), query_node, left_dot_product);
    }
    if (dot_product >= threshold) {
        this->search_single_query_point(data_node->right.get(), query_node, dot_product);
    }
}

void GT::GroupTestingNN::search_double_subtree(GT::tree_node_t *data_node, 
        GT::tree_node_t *query_node, double dot_product) {
    if (data_node->left == nullptr) {
        search_single_data_point(data_node, query_node, dot_product);
        return;
    }
    if (query_node->left == nullptr) {
        search_single_query_point(data_node, query_node, dot_product);
        return;
    }
    if (this->gen() % 3) {
        double left_dot_product = data_node->sum_vec * query_node->left->sum_vec;
        ...
        this->num_dot_products++;
        if (left_dot_product >= threshold) {
            search_double_subtree(data_node, query_node->left.get(), left_dot_product);
        }
        if (dot_product >= threshold) {
            search_double_subtree(data_node, query_node->right.get(), dot_product);
        }
    } else {
        double left_dot_product = data_node->left->sum_vec * query_node->sum_vec;
        dot_product -= left_dot_product;
        this->num_dot_products++;
        if (left_dot_product >= threshold) {
            search_double_subtree(data_node->left.get(), query_node, left_dot_product);
        }
        if (dot_product >= threshold) {
            search_double_subtree(data_node->right.get(), query_node, dot_product);
        }
    }
}
...
GT::tree_node_t GT::build_index_tree(GT::sparse_vec_t *matrix, size_t __size, size_t __start_index) {
    if (__size == 0) {
    ...
    if (__size == 1) {
        node.sum_vec = *matrix;
        node.index = __start_index;
        return node;
    }
    size_t mid = __size / 2;
    node.left = make_unique<GT::tree_node_t>(GT::build_index_tree(matrix, mid, __start_index));
    node.right = make_unique<GT::tree_node_t>(GT::build_index_tree(matrix + mid, __size - mid, __start_index + mid));
