...
namespace GTnn {
    ...
    public:
        ...
        pair<map<uint, vector<uint>>, size_t> search(sparse_mat_t &queries);
    ...
    protected:
        ...
        size_t dimention;
        size_t k_val;
        pair<vector<vector<uint>>, size_t> search_threshold(sparse_mat_t &queries, double threshold);
        pair<result_elem_t, size_t> search_pool(
            sparse_single_index_t<NUM_LEVELS> &pool, sparse_single_index_t<INVERTED_LEVELS> &qpool, 
...
pair<map<uint, vector<uint>>, size_t> GTnn::GroupTestingNN::search(GTnn::sparse_mat_t &query_set) {
    pair<map<uint, vector<uint>>, size_t> result;
    if (this->data_index.size() == 0 || query_set.size() == 0) {
        return result;
    }
    vector<uint> pending_queries(query_set.size(), 0);
    iota(pending_queries.begin(), pending_queries.end(), 0);
    GTnn::sparse_mat_t queries;
    for (double threshold : {0.6, 0.4, 0.2, 1.0e-6}) {
        queries.clear();
        for (uint q : pending_queries) {
            queries.push_back(query_set[q]);
        }
    ...
    for (uint i = 0; i < query_set.size(); i++) {
        ...
        for (uint j : result.first[i]) {
            sorted_results.push_back({GTnn::dot_product(query_set[i], this->data_set[j]), j});
        }
...
pair<vector<vector<uint>>, size_t> GTnn::GroupTestingNN::search_threshold(
    GTnn::sparse_mat_t &queries, double threshold
) {
