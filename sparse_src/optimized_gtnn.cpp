...
//     cout << "Naive: number of dot products: " << gt.size() * query_set.size() << endl;
//     return 0;
// }
...
int main2(int argc, char ** argv) {
    ...
    auto stop = high_resolution_clock::now();
    net_search_time = duration_cast<microseconds>(stop - start).count() / (1.0e+3);
    cout << "Mean search time: " << net_search_time / query_set.size() << " ms" << endl;
    cout << "Net number of dot products: " << net_num_dots << endl;

    for (size_t i = 0; i < query_set.size(); i++) {
        array<double, 2> time_and_recall = gt.verify_results(query_set[i], result_set[i]);
    ...
    cout << "Mean search time: " << net_search_time / query_set.size() << " ms" << endl;
    cout << "Net number of dot products: " << net_num_dots << endl;
    cout << "Mean naive time: " << net_naive_time / query_set.size() << " ms" << endl;
    cout << "Mean recall: " << mean_recall / query_set.size() << endl;
    cout << "Naive: number of dot products: " << gt.size() * query_set.size() << endl;
    return 0;
}
