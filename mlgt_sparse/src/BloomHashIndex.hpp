...
public:
    ...
    BloomHashIndex(
        ...
        Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> data_eigen(num_items_, dimension);
    ...
    BloomHashIndex(
        ...
        uint threshold = BLOOM_THRESHOLD,
        int debug = 0
    ) : BloomHashFunction(dim, num_hashes, num_bits, threshold, debug),
        index_(num_hashes, threshold) {
     ...
     */
    inline bool matches_hashes(const vector<uint>& query_hashes) const {
     ...
     */
    inline bool matches(const Eigen::VectorXf &query) const {
        return matches_hashes(BloomHashFunction::operator()(query));
    }

    // Must implement/override Hasher::operator()
    virtual vector<uint> operator()(const float* data, const uint32_t* indices, uint32_t nnz) const override {
        return BloomHashFunction::operator()(data, indices, nnz);
    }

    virtual vector<uint> operator()(const Eigen::VectorXf& q) const override {
        return BloomHashFunction::operator()(q);
    }
};
