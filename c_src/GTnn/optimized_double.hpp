...
#define DOUBLE_NUM_LEVELS 7
#define DOUBLE_INVERTED_LEVELS 5

namespace GTnn {
    using result_elem_t = array<vector<uint>, (1 << DOUBLE_INVERTED_LEVELS)>;
    template <uint N = DOUBLE_NUM_LEVELS> 
        using double_elem_t = array<vector_t, (1 << (N + 1))>;
    template <uint N = DOUBLE_NUM_LEVELS> 
        using double_index_t = vector<double_elem_t<N>>;
    template <uint N = DOUBLE_NUM_LEVELS> 
        double_index_t<N> build_optimized_double_index(matrix_t &matrix);

    class OptimizedDoubleNN {
    public:
        OptimizedDoubleNN(string file_name, double __threshold = 0.8);
        pair<vector<vector<uint>>, size_t> search(matrix_t &queries);
        array<double, 3> verify_results(
            vector_t &query, 
            vector<uint> &src_result
        );
        size_t size() {
            return this->data_set.rows();
        }

    protected:
        matrix_t data_set;
        double_index_t<DOUBLE_NUM_LEVELS> data_index;
        size_t dimention;
        double threshold;
        pair<result_elem_t, size_t> search_pool(
            double_elem_t<DOUBLE_NUM_LEVELS> &pool, 
            double_elem_t<DOUBLE_INVERTED_LEVELS> &qpool, uint &pool_index
        );
    };
};

template <uint N>
GTnn::double_index_t<N> GTnn::build_optimized_double_index(
    GTnn::matrix_t &matrix
) {
    const uint num_vectors = matrix.rows();
    const uint num_indices = (num_vectors + (1 << N) - 1) / (1 << N);
    GTnn::double_index_t<N> data_index(num_indices);
    for (uint i = 0; i < num_indices; i++) {
        ...
        for (uint j = 0; j < (1 << N); j++) {
            ...
            if (index < num_vectors) {
                ...
                data_index[i][(1 << N) + j] = vector_t::Zero(matrix.cols());
            }
        }
        for (int n = N - 1; n >= 0; n--) {
            for (uint j = 0; j < static_cast<uint>(1 << n); j++) {
                const uint index = (1 << n) + j;
                data_index[i][index] = data_index[i][2 * index] + 
                                        data_index[i][2 * index + 1];
            }
        }
    }
    return data_index;
...
GTnn::OptimizedDoubleNN::OptimizedDoubleNN(
    string file_name, 
    double __threshold
) {
    this->dimention = GTnn::extract_matrix(file_name, this->data_set);
    this->threshold = __threshold;
    this->data_index = GTnn::build_optimized_double_index<DOUBLE_NUM_LEVELS>(
        this->data_set
    );
}

pair<vector<vector<uint>>, size_t> GTnn::OptimizedDoubleNN::search(
    GTnn::matrix_t &queries
) {
    GTnn::double_index_t<DOUBLE_INVERTED_LEVELS> 
    query_pools = GTnn::build_optimized_double_index<DOUBLE_INVERTED_LEVELS>(
        queries
    );
    vector<pair<GTnn::result_elem_t, size_t>> async_results(
    ...
    auto worker = [this, &async_results, &query_pools] (uint start, uint end) {
        for (uint pool_index = start; pool_index < end; pool_index++) {
            for (uint qpool_index = 0; 
                qpool_index < query_pools.size(); 
                qpool_index++)
            {
                async_results[
                    pool_index * query_pools.size() + qpool_index
                ] = this->search_pool(
                    this->data_index[pool_index], 
                    query_pools[qpool_index], pool_index
                );
            }
        }
    };
    ...
    pair<vector<vector<uint>>, size_t> result = {
        vector<vector<uint>>(queries.rows()), 
        0
    };
    for (uint pool_index = 0; 
        pool_index < this->data_index.size(); 
        pool_index++)
    {
        for (uint qpool_index = 0; 
            qpool_index < query_pools.size(); 
            qpool_index++) 
        {
            auto &async_result = async_results[
                pool_index * query_pools.size() + qpool_index
            ].first;
            result.second += async_results[
                pool_index * query_pools.size() + qpool_index
            ].second;
            for (uint i = 0; i < async_result.size(); i++) {
                uint qidx = i + qpool_index * (1 << DOUBLE_INVERTED_LEVELS);
...
pair<GTnn::result_elem_t, size_t> GTnn::OptimizedDoubleNN::search_pool(
        GTnn::double_elem_t<DOUBLE_NUM_LEVELS> &pool, 
        GTnn::double_elem_t<DOUBLE_INVERTED_LEVELS> &qpool, 
        uint &pool_index
) {
    pair<GTnn::result_elem_t, size_t> result = {GTnn::result_elem_t(), 1};
    array<double, 2 * (DOUBLE_NUM_LEVELS + DOUBLE_INVERTED_LEVELS) + 1> dots;
    array<uint, 2 * (DOUBLE_NUM_LEVELS + DOUBLE_INVERTED_LEVELS) + 1> idxs;
    array<uint, 2 * (DOUBLE_NUM_LEVELS + DOUBLE_INVERTED_LEVELS) + 1> qidxs;
    uint dot_idx = 1;
    ...
    while (dot_idx > 0) {
        ...
        if (idx >= static_cast<uint>(1 << DOUBLE_NUM_LEVELS)) {
            if (qidx >= static_cast<uint>(1 << DOUBLE_INVERTED_LEVELS)) {
                uint data_idx = idx - (1 << DOUBLE_NUM_LEVELS) + 
                                pool_index * (1 << DOUBLE_NUM_LEVELS);
                uint query_idx = qidx - (1 << DOUBLE_INVERTED_LEVELS);
...
array<double, 3> GTnn::OptimizedDoubleNN::verify_results(
    GTnn::vector_t &query,
    vector<uint> &result
) {
