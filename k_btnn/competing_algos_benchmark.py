...
def benchmark_algo(name, algo_class, data, queries, k, double_group=False, use_threading=True, **kwargs):
    ...
    try:
        ...
        max_memory_used_mb = peak_bytes / (1024 * 1024)
        tracemalloc.stop()
        print(f"Search time: {(search_time*1000):.5f}ms, QPS: {qps:.4f}, Dots: {dots}")
        return res_indices, idx, build_time, search_time, max_memory_used_mb
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_csv", type=str, default="competing_algos_ablation_results.csv", help="CSV filename for benchmark results.")
    ...
    algo_configs = [
        ...
        ("Linscan (budget: 4)", baselines.LinscanWrapper, {"budget": 4}),
        ("Linscan (budget: 6)", baselines.LinscanWrapper, {"budget": 6}),
        ...
        ("Cufe (budget: 4)", baselines.CufeWrapper, {"budget": 4}),
        ("Cufe (budget: 6)", baselines.CufeWrapper, {"budget": 6}),
    ...
    for dataset in datasets_to_run:
        ...
        for step_idx, (N, Nq) in enumerate(zip(num_features, num_queries)):
            ...
            for name, cls, kwargs in algo_configs:
                ...
                if action == "Build":
                    res_indices, idx_obj, b_time, s_time, peak_mem = benchmark_algo(name, cls, current_data, current_queries, args.k, double_group=dg, use_threading=ut, **cfg_kwargs)
                    if idx_obj is not None and res_indices is not None:
                        ...
                        record.update({
                            ...
                            "max memory used": round(peak_mem, 4)
                        })
                else:
                    if name in indexes:
                        ...
                        if hasattr(idx, 'streaming_update'):
                            ...
                            try:
                                ...
                                res_indices = results[0] if isinstance(results, tuple) else results
                                _, peak_bytes = tracemalloc.get_traced_memory()
                                record["max memory used"] = round(peak_bytes / (1024 * 1024), 4)
