...
def make_run_label(prefix: str = "run"):
...
class PeakMemoryMonitor:
    """
    Memray-backed monitor that keeps the original output schema while using Memray's
    Tracker for allocation profiling and a resource fallback for peak RSS sampling.
    """
    ...
    def start(self):
        run_label = make_run_label("memray")
        fd, self._tmp_path = tempfile.mkstemp(prefix=f'{run_label}_', suffix='.bin')
        os.close(fd)
        try:
            os.unlink(self._tmp_path)
        except OSError:
            pass
        self._tracker = memray.Tracker(self._tmp_path)
        self._tracker.__enter__()

        self._start_bytes = self._rss_bytes()
    ...
    def stop(self):
        ...
        end_bytes = self._rss_bytes()
        if end_bytes > self._peak_bytes:
            self._peak_bytes = end_bytes
        if end_bytes < self._min_bytes:
            self._min_bytes = end_bytes
        ...
        reader_cls = getattr(memray, 'FileReader', None)
        if reader_cls is not None and self._tmp_path and os.path.exists(self._tmp_path):
            try:
                reader = reader_cls(self._tmp_path)
                with contextlib.suppress(Exception):
                    self._peak_bytes = max(self._peak_bytes, int(reader.get_high_watermark() or 0))
            except Exception:
                pass
            try:
                os.unlink(self._tmp_path)
...
def run_benchmarks(args):
    ...
    for dataset in datasets_to_run:
        ...
        for step_idx, (N, Nq, action_type) in enumerate(phases):
            ...
            del all_scores, gt_indices, gt_scores_top_k
            gc.collect()
        ...
        for name, cls, kwargs in algo_configs:
            for step_idx, (N, Nq, action_type) in enumerate(phases):
                ...
                record = {
                    "Dataset": dataset, "Phase": phase_num, "Algo": name,
                ...
                if action_type == "Build":
                    ...
                    if idx_obj is not None and res_indices is not None:
                        ...
                        rec = compute_recall_scores(np.array(ground_truths[step_idx]["indices"]), np.array(res_scores))
                    ...
                    if args.streaming:
                        ...
                        if hasattr(idx, 'streaming_update'):
                            ...
                            try:
                                ...
                                record["Recall"] = round(compute_recall_scores(np.array(ground_truths[step_idx]["indices"]), np.array(res_scores)), 4)
...
if __name__ == "__main__":
    ...
    # Run benchmarks
    run_label = make_run_label("bench")
    print(f"Starting benchmark run {run_label}...")
    ...
    if results:
        ...
        if args.output_csv == "benchmark_results.csv":
            csv_name = f"{name_no_ext}_{run_label}{ext}"
        else:
            csv_name = args.output_csv
        csv_file = os.path.join(CUR_DIR, csv_name)
        print(f"\nSaving benchmark results to {csv_file}...")
        with open(csv_file, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=results[0].keys())
            writer.writeheader()
            writer.writerows(results)
        print(f"Results saved → {csv_file}")
        print(f"Dataset(s): {args.dataset}, points: {args.num_features}, queries: {args.num_queries}, k={args.k}")
        print(f"Run label: {run_label}")

        # Generate plots unless skipped
        if not args.skip_plot:
            plot_path = generate_plots(csv_file)
            if plot_path:
                print(f"Plot saved → {plot_path}")
    
    print("\n" + "="*40)
    print(f"Benchmark Complete: {run_label}")
