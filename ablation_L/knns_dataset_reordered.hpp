#ifndef ABLATION_KNNS_DATASET_REORDERED_HPP
#define ABLATION_KNNS_DATASET_REORDERED_HPP
...
template <uint L> struct knns_reordered_index_t {
    ...
    vector<array<uint, (1 << L)>> reorder_maps;
...
// L  = KNNS_TREE_LEVELS  (data-side tree depth, leaf group size = 2^L)
// Lq = KNNS_INVERTED_LEVELS (query-side pool depth, pool size = 2^Lq)
template <uint L, uint Lq>
class KNNReorderedIndexDatasetT {
public:
    using knns_elem_t        = knns_reordered_elem_t<L>;
    using knns_index_t       = knns_reordered_index_t<L>;
    using knns_inverted_index_t = knns_reordered_index_t<Lq>;

    KNNReorderedIndexDatasetT(matrix_t &mat, size_t k = 10) : data_set(mat), k_val(k), index(mat) {
    ...
    pair<vector<uint>, size_t> search(vector_t &query, bool use_threading = true) {
        ...
        while (!search_index.empty()) {
            ...
            if (node_idx >= static_cast<uint>(1 << L)) {
                uint abs_idx = rmap[node_idx - (1 << L)];
            ...
            double right_dot = dot_val - left_dot; // query.dot(tree[right_child]);
    ...
    pair<vector<vector<uint>>, size_t> search_multiple(matrix_t &query_set, bool use_threading = true) {
        ...
        if (!use_threading) {
            ...
            for (uint p = 0; p < num_pools; p++) {
                ...
                for (uint j = 0; j < (1u << Lq); j++) {
                    uint q_abs_idx = qrmap[j];
                    if (q_abs_idx < num_queries)
                        result.first[q_abs_idx] = std::move(raw_results.first[p * (1u << Lq) + j]);
                }
            }
            return result;
        }
        ...
        for (int p = 0; p < (int)num_pools; p++) {
            ...
            async_results[p] = this->search_multiple_internal(single_pool, 1u << Lq);
        }
    ...
    array<double, 2> verify_results(vector_t query, vector<uint> &result) {
        ...
        uint match_count = 0;
        for (uint idx : result)
            if (query.dot(data_set.row(idx)) >= min_true_dot - 1e-9) match_count++;
...
private:
    ...
    pair<vector<pair<double, uint>>, size_t> search_pool_single(
        ...
        while (result.first.size() < this->k_val && !search_pool_index.empty()) {
            ...
            if (n_idx >= static_cast<uint>(1 << L)) {
                uint abs_idx = rmap[n_idx - (1 << L)];
                if (abs_idx != std::numeric_limits<uint>::max())
                    result.first.push_back({dot_val, abs_idx});
                continue;
            }
            uint left = 2 * n_idx; uint right = 2 * n_idx + 1;
            double left_dot  = query.dot(tree[left]);
            double right_dot = dot_val - left_dot; // query.dot(tree[right]);
            search_pool_index.push({left_dot,  t_idx, left});
            search_pool_index.push({right_dot, t_idx, right});
            result.second += 2;
        }
        return result;
    }
    ...
    pair<vector<vector<uint>>, size_t> search_multiple_internal(
        ...
        for (uint tree_idx = 0; tree_idx < this->index.trees.size(); tree_idx++) {
            ...
            for (uint qtree_idx = 0; qtree_idx < query_pools.trees.size(); qtree_idx++) {
                ...
                search_index.push({qtree[1].dot(tree[1]), tree_idx, 1, qtree_idx, 1});
                result.second++;
            }
        }
        ...
        while (finished_queries < active_queries_count && !search_index.empty()) {
            ...
            if (node_idx >= static_cast<uint>(1 << L) &&
                qnode_idx >= static_cast<uint>(1 << Lq)) {
                uint q_rel  = qnode_idx - (1u << Lq);
                uint data_idx = rmap[node_idx - (1u << L)];
                uint local_q  = qtree_idx * (1u << Lq) + q_rel;
                if (data_idx != std::numeric_limits<uint>::max() &&
                    local_q < num_queries_to_fill &&
                    result.first[local_q].size() < this->k_val) {
                    result.first[local_q].push_back(data_idx);
                    if (result.first[local_q].size() == this->k_val) finished_queries++;
                }
                continue;
            }
            if (node_idx >= static_cast<uint>(1 << L)) {
                uint lqc = 2 * qnode_idx, rqc = 2 * qnode_idx + 1;
                double left_dot  = qtree[lqc].dot(tree[node_idx]);
                double right_dot = dot_val - left_dot; // qtree[rqc].dot(tree[node_idx]);
                search_index.push({left_dot,  tree_idx, node_idx, qtree_idx, lqc});
                search_index.push({right_dot, tree_idx, node_idx, qtree_idx, rqc});
                result.second += 2;
            } else if (qnode_idx >= static_cast<uint>(1 << Lq)) {
                uint q_rel   = qnode_idx - (1u << Lq);
                uint local_q = qtree_idx * (1u << Lq) + q_rel;
                if (local_q < num_queries_to_fill && result.first[local_q].size() >= this->k_val) continue;
                uint lc = 2 * node_idx, rc = 2 * node_idx + 1;
                double left_dot  = qtree[qnode_idx].dot(tree[lc]);
                double right_dot = dot_val - left_dot; // qtree[qnode_idx].dot(tree[rc]);
                search_index.push({left_dot,  tree_idx, lc, qtree_idx, qnode_idx});
                search_index.push({right_dot, tree_idx, rc, qtree_idx, qnode_idx});
                result.second += 2;
            } else {
                uint lc = 2 * node_idx,  rc  = 2 * node_idx + 1;
                uint lqc = 2 * qnode_idx, rqc = 2 * qnode_idx + 1;
                double ll_dot = qtree[lqc].dot(tree[lc]);
                double lr_dot = qtree[lqc].dot(tree[rc]);
                double rl_dot = qtree[rqc].dot(tree[lc]);
                double rr_dot = dot_val - ll_dot - lr_dot - rl_dot; // qtree[rqc].dot(tree[rc]);
                search_index.push({ll_dot, tree_idx, lc, qtree_idx, lqc});
                search_index.push({rl_dot, tree_idx, lc, qtree_idx, rqc});
                search_index.push({lr_dot, tree_idx, rc, qtree_idx, lqc});
                search_index.push({rr_dot, tree_idx, rc, qtree_idx, rqc});
                result.second += 4;
            }
        }
...
#endif /* ABLATION_KNNS_DATASET_REORDERED_HPP */
